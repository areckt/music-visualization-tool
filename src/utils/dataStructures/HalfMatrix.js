...
export default class HalfMatrix {
  ...
  constructor(options) {
    ...
    this.numberType =
      getNumberTypeByName(options.numberType) || NumberType.FLOAT32
  ...
  setValue(x, y, value) {
    this.data[((y * y + y) / 2) * this.featureAmount + x * this.featureAmount] =
      value
  }
  setValueNormalized(x, y, value) {
    this.data[((y * y + y) / 2) * this.featureAmount + x * this.featureAmount] =
      value * this.numberType.scale
  }
  setValueNormalizedMirrored(x, y, value) {
    if (x > y) {
      this.data[
        ((x * x + x) / 2) * this.featureAmount + y * this.featureAmount
      ] = value * this.numberType.scale
    } else {
      this.data[
        ((y * y + y) / 2) * this.featureAmount + x * this.featureAmount
      ] = value * this.numberType.scale
    }
  }
  getValue(x, y, f = 0) {
    return this.data[
      ((y * y + y) / 2) * this.featureAmount + x * this.featureAmount + f
    ]
  }
  getValues(x, y) {
    ...
    for (let o = 0; o < this.featureAmount; o++) {
      values[o] =
        this.data[
          ((y * y + y) / 2) * this.featureAmount + x * this.featureAmount + o
        ]
    }
    return values
  }
  ...
  /**
   *
   * @param {*} callback function that returns number (compatible with DataType)
   */
  fill(callback) {
    for (let y = 0; y < this.size; y++) {
    ...
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < y + 1; x++) {
        for (let f = 0; f < this.featureAmount; f++) {
          this.data[cellsBefore + x * this.featureAmount + f] = callback(
            x,
            y,
            f
          )
  ...
  fillFeaturesNormalized(callback) {
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < y + 1; x++) {
        for (let f = 0; f < this.featureAmount; f++) {
          this.data[cellsBefore + x * this.featureAmount + f] =
  ...
  /**
   * Expects normalized number [0,1] and stores this as number of specified type, with 1 being scaled to the number's max
  ...
  getMeanAndStandardDeviation() {
    ...
    for (let y = 0; y < this.size; y++) {
      const cellsBefore = ((y * y + y) / 2) * this.featureAmount
      for (let x = 0; x < y + 1; x++) {
        meanSquared += Math.pow(
          this.data[cellsBefore + x] / this.numberType.scale - mean,
          2
        )
      }
    }
