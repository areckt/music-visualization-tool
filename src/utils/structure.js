...
export function createFixedDurationStructureSegments(
  sampleAmount,
  sampleDuration,
  duration
) {
  ...
  while (start + duration < sampleAmount) {
    structureSegments.push(
      new Section({
        ...
        end: (start + duration) * sampleDuration,
      })
    )
    start += duration
  }
...
export function createSegmentsFromNovelty(
  novelty,
  sampleDuration,
  threshold = 0.1
) {
  ...
  for (let i = 0; i < peaks.length; i++) {
    ...
    const end =
      i < peaks.length - 1
        ? sampleDuration * peaks[i + 1].sample
        : sampleDuration * novelty.length
    const endSample =
      i < peaks.length - 1 ? peaks[i + 1].sample : novelty.length
...
export function computeStructureCandidates(
  ...
  const candidates = []
  // console.log('ASD', structureSegments)
...
export function computeSeparateStructureCandidates(
  ...
  separateSegmentSets.forEach((segments) => {
    const candidates = computeStructureCandidates(
      pathSSM,
      segments,
      minDurationSeconds,
      maxRatio,
...
export function findFittestSections(ssm, segments, strategy = 'classic') {
  ...
  while (sortedCandidates.length > 0 && repeats < segments.length) {
    ...
    const pathFamily = getPathFamily(
      bestSection,
      sampleDuration,
      bestSection.groupID
    )
    ...
    const nonOverlappingExtendedPathFamily = addNonOverlapping(
      pathFamily,
      extendedPathFamily
    )
    const prunedPaths = pruneLowConfidence(
      nonOverlappingExtendedPathFamily,
      0.15
    )
...
export function findMuteDecomposition(
  ...
  while (separateSegmentSets.length > 0 && i < maxRepeats) {
    ...
    if (
      best === null ||
      best[comparisonProperty] <= minFitness ||
      isNaN(best[comparisonProperty])
    ) {
      break
    }
    const groupID = i
    ...
    sortedStructureSections = sortedStructureSections.sort((a, b) =>
      a.start > b.start ? 1 : -1
    )
    if (updateCallback) {
...
export function findSubDecomposition(
  ...
  let ID = 0
  for (let i = 0; i < 20; i++) {
    let groupSections = structureSections.filter(
      (section) => section.groupID === i
    )
    ...
    groupSections = groupSections.filter(
    ...
    do {
      ...
      if (subStructureSection) {
        const subPathFamily = getPathFamily(
          subStructureSection,
          sampleDuration,
          ID
        )
        ...
        const subFamily = pruneLowConfidence(
          nonOverlappingExtendedSubPathFamily,
          0.1
        )
        ...
        if (remainingSections.length > 0) {
          ...
          if (j > 0) {
            const candidates = []
            const scoreMatrixBuffer = pathExtraction.createScoreMatrixBuffer(
              pathSSM.getSampleAmount()
            )
            remainingSections.forEach((section) => {
              const startInSamples = Math.floor(
                section.start / pathSSM.sampleDuration
              )
              const endInSamples = Math.floor(
                section.end / pathSSM.sampleDuration
              )
              candidates.push(
                createCandidate(
                  pathSSM,
            ...
            while (remainingSections.length > 0 && repeats < 1) {
              ...
              subStructureSections.push(...subFamily)
              remainingSections = subtractSegments(
                remainingSections,
                subStructureSections
              )
...
export function findSubstructure(
  ...
  const maxSubSizeSamples = Math.floor(
    maxDurationSeconds / pathSSM.sampleDuration
  )
  const minSubSizeSamples = Math.floor(
    minDurationSeconds / pathSSM.sampleDuration
  )
  ...
  courseSections.forEach((section) => {
    ...
    for (
      let start = startInSamples;
      start < endInSamples - maxSubSizeSamples;
      start += step
    ) {
  ...
  // Assume that if the group sections are same amount as subfamily, the subfamily is just pointing out largest fraction of group section
  const candidatesFiltered = candidates.filter(
    (c) =>
      c.pathFamily.length !== courseSections.length && c.fitness >= minFitness
  )
...
export function findGreedyDecomposition(
  ...
  while (separateSegmentSets.length > 0 && i < maxRepeats) {
    ...
    structureSections = structureSections.sort((a, b) =>
      a.start > b.start ? 1 : -1
    )
...
function getPathFamily(section, sampleDuration, groupID) {
  ...
  section.pathFamily.forEach((path, index) => {
    ...
    const pathConfidence = section.pathFamilyScores
      ? section.pathFamilyScores[index]
      : section.pathScores[index]
...
function getExtendedPathFamily(
  section,
  sampleDuration,
  groupID,
  pathSSM,
...
function addNonOverlapping(sectionsA, sectionsB, overlapRatioThreshold = 0) {
  ...
  const sortedSectionsB = sectionsB.sort((a, b) =>
  ...
  for (
    ...
    for (let endOffset = -sampleOffset; endOffset < sampleOffset; endOffset++) {
      ...
      if (
        ...
        (allowOverlap ||
          !overlapWithStructureSections(
            { start: startInSeconds, end: endInSeconds },
            structureSections
          ))
      ) {
        const segmentPathFamilyInfo =
          pathExtraction.computeSegmentPathFamilyInfo(
            pathSSM,
            start,
            end,
            scoreMatrixBuffer,
...
function subtractStructureFromSegments(
  separateSegmentSets,
  sortedStructureSections,
  trackEnd,
  smallestAllowedSize
  ...
  const freeRanges = getFreeRanges(
    sortedStructureSections,
    trackEnd,
    smallestAllowedSize
  )
...
function getFreeRanges(structureSections, trackEnd, smallestAllowedSize) {
  ...
  return freeRanges.filter(
    (range) => range[1] - range[0] >= smallestAllowedSize
  )
}
...
export function groupSimilarSegments(segments, pathSSM, maxDistance = 0.8) {
  const groupedSegments = cloneDeep(segments)
  ...
  while (groupIDed < segments.length) {
    ...
    groupedSegments.forEach((segmentA) => {
      groupedSegments.forEach((segmentB) => {
        if (
          (segmentA.groupID === undefined || segmentB.groupID === undefined) &&
          segmentA !== segmentB
        ) {
          const distance = pathExtraction.getDistanceBetween(
            segmentA,
            segmentB,
            pathSSM
          )
    ...
    if (minDistance > maxDistance) {
      if (
        bestSegmentA.groupID === undefined &&
        bestSegmentB.groupID === undefined
      ) {
...
export function MDSColorSegments(
  segments,
  pathSSM,
  strategy = 'overlap',
  coloringStrategy,
  ...
  const distanceMatrix = pathExtraction.getDistanceMatrix(
    segments,
    pathSSM,
    strategy
  )
  const segmentsColored = MDSColorGivenDistanceMatrix(
    segments,
    distanceMatrix,
    coloringStrategy
  )
  ...
  if (colorBoth) {
    const distanceMatrixCategorical = pathExtraction.getDistanceMatrix(
      segments,
      pathSSM,
      strategy,
      0
    ...
    for (let i = 0; i < segmentsColored.length; i++) {
      segmentsColored[i].catColorAngle =
        segmentsColoredCategorical[i].colorAngle
      segmentsColored[i].catColorRadius =
        segmentsColoredCategorical[i].colorRadius
      segmentsColored[i].catMdsFeature =
        segmentsColoredCategorical[i].mdsFeature
    }
  }
...
export function MDSColorSegmentsPerGroup(
  segments,
  pathSSM,
  strategy = 'overlap',
  coloringStrategy
  ...
  segmentGroups.forEach((segmentGroup) => {
    ...
    const groupSegmentsColored = MDSColorGivenDistanceMatrix(
      segmentGroup,
      distanceMatrix,
      coloringStrategy
    )
...
export function MDSColorGivenDistanceMatrix(
  segments,
  distanceMatrix,
  coloringStrategy
) {
  ...
  coloredSegments.sort((a, b) => {
    return a.colorAngle > b.colorAngle
      ? 1
      : b.colorAngle > a.colorAngle
      ? -1
      : 0
  })
  ...
  let largestGap =
    1 -
    coloredSegments[coloredSegments.length - 1].colorAngle +
    coloredSegments[0].colorAngle
  let largestGapAngle = coloredSegments[0].colorAngle
  for (let i = 1; i < coloredSegments.length; i++) {
    const gap =
      coloredSegments[i].colorAngle - coloredSegments[i - 1].colorAngle
    if (gap > largestGap) {
      largestGap = gap
    ...
    groupIDSections.forEach((groupSection) => {
      if (strategy === 'no-overlap') {
        ...
        squashed.forEach((squashedSection, index) => {
          if (
            groupSection.overlaps(squashedSection) &&
            squashedSection !== groupSection
          ) {
            overlap = true
            if (
        ...
        if (
          thereIsSpace &&
          newGroupSection.end - newGroupSection.start > minSize
        ) {
...
export function MDSColorTimbreSegmentsWithSSM(blurredTimbreSSM, segments) {
  ...
  segments.forEach((segment) => {
    ...
    const middleSample = Math.floor(
      middleSeconds / blurredTimbreSSM.getSampleDuration()
    )
...
export function MDSColorTimbreSegmentsWithFeatures(
  timbreFeatures,
  segments,
  sampleDuration,
  strategy
...
export function clusterTimbreSegmentsWithFeatures(
  timbreFeatures,
  segments,
  sampleDuration
) {
...
export function processTimbreSegments(
  timbreFeatures,
  segments,
  sampleDuration,
  strategy
) {
  const mdsColoredSegments = MDSColorTimbreSegmentsWithFeatures(
    timbreFeatures,
    segments,
    sampleDuration,
    strategy
...
export function sortGroupByCoverage(sections) {
  ...
  sections.forEach((section) => {
    if (groupCoverage[section.groupID] === undefined) {
      groupCoverage[section.groupID] = {
        groupID: section.groupID,
        coverage: section.end - section.start,
      }
    } else {
  ...
  sections.forEach((section) => {
    const newGroup = groupCoverage.findIndex(
      (group) => group.groupID === section.groupID
    )
...
export function createSeparators(coloredSegments, pathSSM) {
  const coloredSegmentsSorted = coloredSegments.sort(
    (a, b) => a.start - b.start
  )
  const separators = []
  ...
  for (let i = 0; i < coloredSegments.length; i++) {
    ...
    if (i > 0) {
      ...
      const angleDistance = Math.abs(
        segment.colorAngle - coloredSegmentsSorted[i - 1].colorAngle
      )
      distanceFromPreviousSegment =
        Math.min(angleDistance, 1 - angleDistance) * 2
    }
