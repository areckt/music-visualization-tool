...
export default class Features {
  ...
  constructor(analysisData, options = {}) {
    ...
    this.sampleAmount = Math.min(
      Math.ceil(this.duration / options.sampleDuration),
      options.samples
    )
    ...
    this.fastSampledPitch = this.sample('pitches', {
      sampleDuration: this.fastSampleDuration,
    })
  ...
  calculateMaxMin() {
    this.segments.forEach((segment) => {
      for (let i = 0; i < 12; i++) {
        this.timbreMax[i] = Math.max(
          this.timbreMax[i],
          segment.getOriginalTimbres()[i]
        )
        this.timbreMin[i] = Math.min(
          this.timbreMin[i],
          segment.getOriginalTimbres()[i]
        )
      }
    ...
    for (let i = 0; i < 12; i++) {
      this.timbreBiggest[i] = Math.max(
        Math.abs(this.timbreMax[i]),
        Math.abs(this.timbreMin[i])
      )
      this.timbreTotalBiggest = Math.max(
        this.timbreTotalBiggest,
        this.timbreBiggest[i]
      )
    }
  }
    ...
    this.segments.forEach((s, i) => {
      ...
      const nextSegmentStartLoudness =
        i + 1 < this.segments.length
          ? this.segments[i + 1].getLoudnessFeatures()[0]
          : 0
      this.processed.avgLoudness.push(
        s.getAverageLoudness(nextSegmentStartLoudness)
      )
      this.processed.dynamics.push(
        s.getAverageLoudness(nextSegmentStartLoudness)
      )
      ...
      for (let t = 0; t < 12; t++) {
        if (t == 0) {
          const timbre =
            this.processed.avgLoudness[this.processed.avgLoudness.length - 1] -
            0.5
          timbres.push(timbre)
        } else {
          const timbre =
            (1 - timbreNormalizationAmount) * s.timbres[t] +
            timbreNormalizationAmount * s.timbresScaled[t]
          timbres.push(timbre)
        }
      }
  ...
  processDirectLoudness() {
    this.directLoudnessAmount = Math.floor(
      this.duration / this.directLoudnessSampleDuration
    )
    ...
    for (let i = 0; i < this.directLoudnessAmount; i++) {
      ...
      this.directLoudness[i] = this.raw.loudness[segmentIndex][1]
    }
  }
  ...
  processSamples() {
    ...
    for (let i = 0; i < this.sampleAmount; i++) {
      this.sampled.chords[i] = chordDetection.getPopChord(
        this.sampled.pitches[i]
      )
      this.sampled.majorminor[i] = chordDetection.getMajorMinorNess(
        this.sampled.pitches[i]
      )
    }
    ...
    this.processed.dynamics = this.processed.dynamics.map((dynamic) => {
      return (
        (dynamic / this.maxLoudness) * (1 - this.dynamicsBase) +
        this.dynamicsBase
      )
    })
  ...
  sample(feature, options) {
    ...
    if (options.sampleDuration) {
      // make sure the length of the track is divisible by sampleDuration
    ...
    for (let s = 0; s < sampleAmount; s++) {
      ...
      if (this.segments[i].getEnd() > sampleEnd) {
        averageFeature.forEach(
          (val, f) => (averageFeature[f] = val + this.segments[i][feature][f])
        )
      ...
      if (this.segments[i].getEnd() > sampleStart) {
        const weight =
          (this.segments[i].getEnd() - sampleStart) / sampleDuration
        averageFeature.forEach(
          (val, f) =>
            (averageFeature[f] = val + weight * this.segments[i][feature][f])
        )
        i++
      }
      ...
      while (
        ...
        this.segments[i].getEnd() < sampleEnd
      ) {
  ...
  // Transform processed features into evenly discretized features
  sampleFeatures() {
    // fill sample start duration
    for (let i = 0; i < this.sampleAmount - 1; i++) {
      this.sampleStartDuration.push([
        i * this.sampleDuration,
        this.sampleDuration,
      ])
    }

    // last sample is shorter than the others
    const lastSampleStart = (this.sampleAmount - 1) * this.sampleDuration
    this.sampleStartDuration.push([
      lastSampleStart,
      this.duration - lastSampleStart,
    ])
    ...
    this.segments.forEach((segment, segmentIndex) => {
      ...
      const sampleRangeStartIndex = Math.max(
        0,
        Math.floor(
          (segment.start - blurOutsideSampleDuration) / this.sampleDuration
        )
      )
      const sampleRangeEndIndex = Math.min(
        this.sampleAmount - 1,
        Math.floor(
          (segmentEnd + blurOutsideSampleDuration) / this.sampleDuration
        )
      )
      ...
      if (rangeSize >= 1) {
        ...
        const sampleBlurEnd =
          firstSample[0] + firstSample[1] + blurOutsideSampleDuration
        const firstSampleOverlap = sampleBlurEnd - segment.start
        this.addFeaturesScaled(
          sampleRangeStartIndex,
          segmentIndex,
          firstSampleOverlap
        )
      }
      if (rangeSize >= 1) {
        ...
        const sampleBlurStart =
          this.sampleStartDuration[sampleRangeEndIndex][0] -
          blurOutsideSampleDuration
    ...
    this.divideFeatures(
      this.sampleAmount - 1,
      this.sampleStartDuration[this.sampleAmount - 1][1] +
        blurOutsideSampleDuration
    )
  ...
  downSampleTimbre(amount) {
    ...
    for (let i = 0; i < downSampleAmount; i++) {
      ...
      for (let f = 0; f < 12; f++) {
        summedTimbre[f] +=
          this.sampled.timbres[startSample][f] * startSampleWeight
    ...
    for (const featureName in this.processed) {
      ...
      if (featureSize) {
        for (let s = 0; s < this.sampleAmount; s++) {
          this.sampled[featureName][s] = new Array(
            this.processed[featureName][0].length
          ).fill(0)
        }
      } else {
        for (let s = 0; s < this.sampleAmount; s++) {
    ...
    for (const featureName in this.processed) {
      ...
      if (featureSize) {
        for (let i = 0; i < featureSize; i++) {
          this.sampled[featureName][sampleIndex][i] =
            this.processed[featureName][segmentIndex][i] * scalar
        }
      } else {
        this.sampled[featureName][sampleIndex] =
          this.processed[featureName][segmentIndex] * scalar
      }
    }
  }
