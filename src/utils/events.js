...
export function detectAverageWindow(
  features,
  sampleDuration,
  windowSizeSeconds,
  threshold
) {
  const uniquenessFeature = uniqueness.computeFromFeatures(
    features,
    sampleDuration,
    windowSizeSeconds
  )
...
export function computeEvents(
  ...
  const colsumDiff = csFeature.map(
    (colsumVal, index) => colsumVal * difFeatureRaw[index]
  )
...
export function splitAverageDifferenceFeature(features, size, offset = 2) {
  const [leftBlur, rightBlur] = filter.splitGaussianBlurFeatures(
    features,
    size,
    offset
  )
    ...
    for (let j = 0; j < size; j++) {
      if (i - offset - j >= 0) {
        minDistance = Math.min(
          minDistance,
          1 - similarity.cosine(features[i], features[i - offset - j])
        )
      }
