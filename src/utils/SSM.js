  ...
  ssm.fillFeaturesNormalized((x, y, f) => {
    if (similarityFunction === 'cosine') {
      return (
        Math.max(
          0,
          sim.cosineTransposed(features[x], features[y], f) - threshold
        ) /
        (1 - threshold)
      )
      ...
      return (
        Math.max(
          ...
          Math.min(
            1,
            sim.euclidianPitchTransposed(features[x], features[y], f) -
              threshold
          )
        ) /
        (1 - threshold)
      ...
      return (
        Math.max(
          0,
          Math.min(1, sim.euclidianTimbre(features[x], features[y]) - threshold)
        ) /
        (1 - threshold)
      )
    }
...
export function enhanceSSM(ssm, options, allPitches = false) {
  const blurLength =
    options.blurLength || Math.round(options.blurTime / ssm.sampleDuration) || 4
  ...
  for (const tempoRatio of tempoRatios) {
    ...
    if (strategy === 'linmed')
      enhancementPasses.push(
        medianSmoothing(
          linearSmoothing(ssm, blurLength, tempoRatio),
          blurLength * 1.5,
          tempoRatio
        )
      )
    if (strategy === 'gaussmed')
      enhancementPasses.push(
        medianSmoothing(
          gaussianSmoothing(ssm, blurLength, tempoRatio),
          blurLength * 1.5,
          tempoRatio
        )
      )
    if (strategy === 'med')
      enhancementPasses.push(medianSmoothing(ssm, blurLength, tempoRatio))
  }
  ...
  smoothedSSM.fillFeatures((x, y, f) => {
    ...
    for (let i = -blur; i < 1 + blur; i++) {
      if (ssm.hasCell(x + i, y + tempos[i + blur])) {
        sum +=
          ssm.getValue(x + i, y + tempos[i + blur], f) *
          (blur + 1 - Math.abs(i))
      }
    }
    return sum / total
  ...
  smoothedSSM.fillFeaturesNormalized((x, y, f) => {
    ...
    for (let offset = -l; offset <= l; offset++) {
      if (ssm.hasCell(x + offset, y + tempos[offset + l])) {
        const value = ssm.getValueNormalized(
          x + offset,
          y + tempos[offset + l],
          f
        )
...
function gauss(blur, sigma = 2) {
  ...
  for (let i = -blur; i < 1 + blur; i++) {
    gauss[i + blur] =
      ...
      Math.exp(-Math.pow(i, 2) / (2 * Math.pow(sigma, 2)))
  }
  return gauss
}
...
export function autoThreshold(ssm, percentage) {
  ...
  for (let i = 0; i < ssm.length; i++) {
    thresholdSSM.data[i] =
      Math.min(
        Math.max(ssm.data[i] / typeScale - thresholdValue, 0) /
          (1 - thresholdValue),
        1
      ) * typeScale
  }
...
export function multiply(ssm, factor) {
  let multipliedSSM
  if (ssm instanceof Matrix) {
    multipliedSSM = Matrix.from(ssm)
  } else {
    multipliedSSM = HalfMatrix.from(ssm)
  }

  multipliedSSM.fillNormalized((x, y) => {
...
export function rowColumnAutoThresholdBinary(
  ssm,
  percentageRow,
  percentageCol = percentageRow
) {
  const typeScale = ssm.numberType.scale

  const rowBinaryMatrix = new HalfMatrix({
    size: ssm.size,
    numberType: NumberType.UINT8,
  })
  const colBinaryMatrix = new HalfMatrix({
    size: ssm.size,
    numberType: NumberType.UINT8,
  })
  ...
  thresholdSSM.fill((x, y) => {
    if (
      rowBinaryMatrix.getValue(x, y) > 0 &&
      colBinaryMatrix.getValue(x, y) > 0
    ) {
...
export function rowColumnAutoThreshold(
  ssm,
  percentageRow,
  percentageCol = percentageRow
) {
    ...
    for (let col = 0; col <= row; col++) {
      if (ssm.getValue(col, row) >= thresholdValue) {
        rowBinaryMatrix.setValue(
          ...
          row,
          Math.min(
            (Math.max(ssm.getValue(col, row) - thresholdValue, 0) /
              (typeScale - thresholdValue)) *
              typeScale,
            typeScale
          )
        )
...
export function binarize(matrix, threshold = 0.5) {
  ...
  if (matrix instanceof Matrix) {
    ...
    binaryMatrix = HalfMatrix.from(matrix, {
...
export function soloAnd(ssm, sectionsInSamples, rowcol = 'rowcol') {
  ...
  soloSSM.fill((x, y) => {
    ...
    return (inSelectionX || !col) && (inSelectionY || !row)
      ? ssm.getValueMirrored(x, y)
      : 0
  })
  ...
  sectionsInSamples.forEach((sectionA) => {
    sectionsInSamples.forEach((sectionB) => {
      if (sectionA !== sectionB) {
        ...
        const duration = yDuration / xDuration
        for (let i = 0; i < xDuration; i++) {
          const x = startX + i
          const y = startY + i * duration
