export function computeFitness(
  ...
  const normalizedScore = Math.max(
    0,
    (score - width) / (pathFamilyLength + error)
  )
  ...
  // fitness
  let fitness =
    (2 * normalizedScore * normalizedCoverage) /
    (normalizedScore + normalizedCoverage + error)
  ...
  return {
    ...
    normalizedCoverage,
    pathFamilyLength,
    prunedPathFamily: pathFamily,
  }
}

export function computeCustomFitness(
  pathFamily,
  pathScores,
  score,
  sampleAmount,
...
export function computeFineFitness(
  pathFamily,
  pathScores,
  score,
  sampleAmount,
  ...
  let fitness =
    ...
    (normalizedScore * normalizedCoverage +
      normalizedCoverage * pathAmount +
      pathAmount * normalizedScore)
...
export function computeCustomPrunedFitness(
  pathFamily,
  pathScores,
  score,
  sampleAmount,
  ...
  const [prunedPathFamily, prunedPathScores, totalScore] = prunePathFamily(
    pathFamily,
    pathScores,
    width,
    0.25,
...
export function computePrunedFitness(
  pathFamily,
  pathScores,
  score,
  sampleAmount,
  ...
  const normalizedCoverage = (coverage - width) / (sampleAmount + error)
  const normalizedNonInducedCoverage =
    (pathFamilyLength - width) / (sampleAmount + error)

  // fitness
  let fitness =
    (2 * normalizedScore * normalizedNonInducedCoverage) /
    (normalizedScore + normalizedNonInducedCoverage + error)
...
export function prunePathFamily(
  pathFamily,
  pathScores,
  width,
  smallestRatio = 0.3,
  ...
  for (let p = 0; p < pathLength; p++) {
    ...
    if (
      score > smallestRatio * secondBestScore &&
      score / width > smallestScore
    ) {
