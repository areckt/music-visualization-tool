  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x <= y; x++) {
      ...
      const gauss = Math.exp(
        -Math.pow(taper, 2) *
          (Math.pow(x - (size - subSize) + 0.5, 2) +
            Math.pow(y - subSize + 0.5, 2))
      )
...
export function findPeaks(novelty) {
  ...
  for (let i = 0; i < maxima.length; i++) {
    ...
    const rightMinima =
      i >= minima.length ? { sample: novelty.length - 1, height: 0 } : minima[i]
    let smallestMinima
...
export function normalizeByColumnDensity(matrix) {
  ...
  const normalizedMatrix = Matrix.from(matrix, {
    numberType: Matrix.NumberType.FLOAT32,
  })
  let max = 0
