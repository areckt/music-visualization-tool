...
import { mul, pow, div, add, transpose, svd, sqrt } from './numeric'

// const { Matrix, pseudoInverse } = require('ml-matrix')
import { Matrix, pseudoInverse } from 'ml-matrix'

export function getMDSFeature(distanceMatrix) {
...
export function getMDSCoordinatesSamples(features, strategy) {
  ...
  switch (strategy) {
    case 'Classic':
      ...
      break
    case 'GD':
      coords = getMdsCoordinatesWithGradientDescentMatrix(
        new Matrix(distanceMatrix.getNestedArray()),
        {
          lr: 5,
        }
      ).coordinates.data
...
export function getMDSCoordinates(distanceMatrix, strategy = 'Classic') {
  ...
  switch (strategy) {
    ...
    case 'GD':
      ...
      break
    case 'GN':
      coords = getMdsCoordinatesWithGaussNewton(
        new Matrix(distanceMatrix.getNestedArray())
      ).coordinates.data
      break
  }
...
function getMdsCoordinatesWithGradientDescentMatrix(
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (lossPerStep.length > 1) {
      ...
      if (
        Math.abs(lossPrev - loss) < minLossDifference ||
        isNaN(loss) ||
        loss > 10
      ) {
...
function classicalMDS(distances, dimensions) {
  ...
  // square distances
  var M = mul(-0.5, pow(distances, 2))

  // double centre the rows/columns
  function mean(A) {
    return div(add.apply(null, A), A.length)
  }
  var rowMeans = mean(M),
    colMeans = mean(transpose(M)),
