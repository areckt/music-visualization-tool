...
export function createScoreMatrixBuffer(sampleAmount) {
  return new Float32Array(sampleAmount * sampleAmount).fill(
    Number.NEGATIVE_INFINITY
  )
}

export function extractPathFamily(ssm, start, end) {
  // console.log('BCA', ssm, start, end)
  let D_tmp
...
export function computeAccumulatedScoreMatrix(ssm, start, end, D) {
  ...
  if (start < 0) console.error('start below 0: ', start)
  if (end > sampleAmount)
    console.error('end above sampleAmount: ', sampleAmount, 'end', end)
  ...
  for (let y = 1; y < height; y++) {
    D[y * width + 0] = Math.max(
      D[(y - 1) * width + 0],
      D[(y - 1) * width + width - 1]
    )
    D[y * width + 1] =
      D[y * width + 0] + penalize(ssm.getValueNormalized(start, y))
...
export function computeOptimalPathFamily(D, width, height) {
  // console.log('ABC', D, width, height)
  const pathFamily = []
  let path = []
...
export function visualizationMatrix(ssm, sampleAmount, start, end) {
  ...
  visualizationMatrix.fill((x, y) => {
    if (x >= length * 3 && x < length * 4) {
      ...
      return (
        (Math.max(0, D[y * width + x - length] - minVal) / (maxVal - minVal)) *
        255
      )
    } else {
...
export function getDistanceBetween(segmentA, segmentB, pathSSM) {
  ...
  const inducedSegmentsA = getInducedSegmentsFromSampleRange(
    startSampleA,
    endSampleA,
    pathSSM
  )
  const inducedSegmentsB = getInducedSegmentsFromSampleRange(
    startSampleB,
    endSampleB,
    pathSSM
  )
...
export function segmentSimilarityDTW(segmentA, segmentB, ssm) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      D[y * width + x] =
        penalize(ssm.getValueNormalized(startSampleB + x, startSampleA + y)) +
        Math.max(diag, right, down)
    }
  }
...
/**
 * Similar if they are approximately repetitions of each other (overlap)
...
let logi = 0
export function computeSegmentPathFamilyInfo(
  pathSSM,
  startInSamples,
  endInSamples,
  scoreMatrixBuffer,
  ...
  const [P, pathScores, score, width] = extractPathFamily(
    pathSSM,
    startInSamples,
    endInSamples,
    scoreMatrixBuffer
  ...
  const fitnessFunction = (
    ...
    switch (strategy) {
      case 'classic':
        return familyFitness.computeFitness(
          P,
          pathScores,
          score,
          sampleAmount,
      ...
      case 'fine':
        return familyFitness.computeFineFitness(
          P,
          pathScores,
          score,
          sampleAmount,
      ...
      case 'pruned':
        return familyFitness.computePrunedFitness(
          P,
          pathScores,
          score,
          sampleAmount,
      ...
      case 'custom':
        return familyFitness.computeCustomFitness(
          P,
          pathScores,
          score,
          sampleAmount,
      ...
      case 'customPruned':
        return familyFitness.computeCustomPrunedFitness(
          P,
          pathScores,
          score,
          sampleAmount,
      ...
      default:
        console.warn('No fitness strategy given, falling back to classic')
        return familyFitness.computeFitness(
          P,
          pathScores,
          score,
...
export function getDistanceMatrix(segments, pathSSM, strategy, kappa = 0.7) {
  ...
  segments.forEach((segment) => {
    ...
    const inducedSegments = getInducedSegmentsFromSampleRange(
      sampleStart,
      sampleEnd,
      pathSSM
    )
  ...
  distanceMatrix.fill((x, y) => {
    ...
    switch (strategy) {
      case 'DTW':
        dist =
          (1 - segmentSimilarityDTW(segments[x], segments[y], pathSSM)) *
          (sameGroup ? kappa : 1)
        ...
        break
      case 'overlap':
        dist = segmentDistanceOverlap(
          segmentInducedSegments[x],
          segmentInducedSegments[y]
        )
        break
    }
    return dist
